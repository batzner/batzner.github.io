...
function displayError(message) {
    message = message || "Something went wrong with the Google Maps API. Please try reloading" + " the page";
    ...
    for (var i = 0; i < cities.length; i++) {
        ...
        for (var j = 0; j < cities.length; j++) {
            ...
            if (i != j) {
                ...
                if (distances[i][j] < distanceRange.min) distanceRange.min = distances[i][j];
                if (distances[i][j] > distanceRange.max) distanceRange.max = distances[i][j];
            }
        }
    }
...
function setMarker(map, name, position) {
    ...
    if (map == cityMap) cityMarkers.push(marker);else coordinateMarkers.push(marker);
}
            ...
            for (var i = 0; i < response.rows.length; i++) {
                ...
                for (var j = 0; j < row.elements.length; j++) {
                    ...
                    if (element.status == google.maps.DistanceMatrixStatus.OK) {
                        ...
                        if (element.duration.value < durationRange.min) durationRange.min = element.duration.value;
                        if (element.duration.value > durationRange.max) durationRange.max = element.duration.value;
                    } else {
                        displayError("GoogleMaps could not find a public transport connection" + " between " + cities[i].name + " and " + cities[j].name + ".");
...
function fillTable(id, matrix, measurement, range) {
    ...
    for (var i = 0; i < matrix.length; i++) {
        ...
        for (var j = 0; j < matrix[i].length; j++) {
            ...
            var text = "-";
            if (i != j) text = measurement == "seconds" ? secondsToText(matrix[i][j]) : Math.round(matrix[i][j]) + " " + measurement;
...
function getCityForPlace(place) {
    ...
    for (var i = 0; i < place.address_components.length; i++) {
        ...
        if (types[0] == "locality") return place.address_components[i].long_name;
    }
