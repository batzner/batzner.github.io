...
var FLIP_DATASET_BUTTON = $('#flip-dataset-card-button');
...
function flipDatasetCard() {
    var showExcerpt = FLIP_DATASET_BUTTON.val() == 'showExcerpt';
    var newText = showExcerpt ? 'Show Results' : 'Show Excerpt';
    var newVal = showExcerpt ? 'showResults' : 'showExcerpt';
    $('#dataset-card').flip(showExcerpt);
    FLIP_DATASET_BUTTON.html(newText).val(newVal);
}

function onDatasetChanged() {
    var datasetName = $('#dataset-dropdown').find('.dropdown-toggle').val();

    // Update the dataset excerpt
    $('#dataset-excerpt').html(EXCERPTS[datasetName]).scrollTop(0);

    // Plot the dataset
    $.getJSON("/assets/posts/char-lm/data/" + datasetName + "/model.trainstate.json").then(function (trainstate) {
...
function plotDatasetLog(log) {
    var lossesTrain = getData(log, 'epochs', 'lossesTrain').filter(function (point) {
        // Ignore and filter outliers completely
        return !(point.y > 30 || point.y < 0.01);
    });
    var lossesValid = getData(log, 'epochs', 'lossesValid').filter(function (point) {
        // Ignore and filter outliers completely
        return !(point.y > 30 || point.y < 0.01);
    });
